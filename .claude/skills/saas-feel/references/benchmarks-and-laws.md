...
Public "100ms rule" (Buchheit's instantaneous threshold) but internally targets **50ms**, newest renderer <32ms (1–2 frames). Local DB + caching (offline), preloads likely-next threads, minimal animation ("no time wasted rendering it"), keyboard-first with a ⌘K palette that shows the shortcut next to each command so it teaches itself. Applied *game design* to productivity.
...
- Use the peak–end rule to decide *where* to spend motion/delight budget: the moment a job is logged, a record is claimed, a payment clears.
