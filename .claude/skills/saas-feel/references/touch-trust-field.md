...
- The _visual_ control can be smaller, but the _tappable_ area must hit the minimum - expand hit area with padding.
...
- **Trust through data, not badges:** show the exact amount + who's charging + for what, _before_ the tap.
...
- **Hierarchy of trust:** don't ask for sensitive info before earning lower-commitment steps. ~18% abandon checkout over security concerns (Baymard); place a recognizable security signal _next to_ card fields, not just the footer.
