...
The paradox: premium products animate **less**, not more. Motion is a tool to explain a state change — not decoration. Stripe's litmus test: *if animations are disabled, the flow should feel broken; otherwise the animation is superfluous.*
...
- **Springs** (react-spring default `{ mass:1, tension:170, friction:26 }`): higher tension = snappier, higher friction = less bounce. Use springs for *movement*, not for color/opacity.
...
- **Origin-aware popovers:** set `transform-origin` to the trigger (Radix/Base UI expose `--radix-*-transform-origin`) so the menu grows *out of* the button.
...
- **`prefers-reduced-motion`: don't kill everything — swap movement for an opacity fade.**
  ...
  @media (prefers-reduced-motion: reduce) {
    .el { animation: fade 0.2s; }  /* was: slide/bounce */
  }
  ```
