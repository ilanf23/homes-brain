...
// a 2px solid indigo border (no top accent bar). Footer sits below the
// card and holds reason + unsubscribe + legal.
//
...
export function renderH1(text: string): string {
  return `<h1 style="margin:0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:24px;font-weight:800;line-height:1.25;letter-spacing:-0.02em;color:#16160f;">${protectBrand(esc(text))}</h1>`;
}
...
export function renderBody(text: string, opts: { marginTop?: number } = {}): string {
  ...
  return `<p style="margin:${mt}px 0 0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:15.5px;line-height:1.55;color:#4a4842;">${protectBrand(esc(text))}</p>`;
}
...
export function renderBodyHtml(html: string, opts: { marginTop?: number } = {}): string {
  const mt = opts.marginTop ?? 14;
  return `<p style="margin:${mt}px 0 0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:15.5px;line-height:1.55;color:#4a4842;">${html}</p>`;
}
...
export function renderDetails(rows: Array<{ label: string; value: string }>): string {
  ...
  const trs = rows
    .map(
      (r) =>
        `<tr><td style="padding:8px 0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#73706a;font-size:13px;width:130px;vertical-align:top;">${protectBrand(esc(r.label))}</td><td style="padding:8px 0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#16160f;font-size:14px;font-weight:600;vertical-align:top;">${protectBrand(esc(r.value))}</td></tr>`,
    )
    .join("");
  return `<div style="margin-top:20px;background:#f7f6f1;border:1px solid #ecebe4;border-radius:14px;padding:14px 18px;"><table role="presentation" style="width:100%;border-collapse:collapse;">${trs}</table></div>`;
}

export function renderCta(url: string, label: string): string {
  const safeUrl = url.replace(/&/g, "&amp;").replace(/"/g, "&quot;");
  return `<div style="margin-top:24px;"><a href="${safeUrl}" style="display:inline-block;background:#473fb0;color:#ffffff;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:15px;font-weight:700;text-decoration:none;border-radius:999px;padding:14px 32px;">${protectBrand(esc(label))}</a></div>`;
}

// Small trailing paragraph, muted. Good for "One tap opens..." or
// "Reply to this email...".
export function renderFinePrint(text: string): string {
  return `<p style="margin:18px 0 0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:12.5px;line-height:1.55;color:#8a877f;">${protectBrand(esc(text))}</p>`;
}
...
function renderFooter(opts: {
  ...
  const reasonLine = opts.reason
    ? `<p style="margin:0 0 12px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.55;color:#8a877f;">${protectBrand(esc(opts.reason))}</p>`
    : "";
  const unsubLine = opts.unsubUrl
    ? `<p style="margin:0 0 8px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.55;color:#8a877f;"><a href="${esc(opts.unsubUrl)}" style="color:#473fb0;text-decoration:underline;">${esc(copy.unsubscribe)}</a> · ${esc(copy.optOut)}</p>`
    : "";
  return `
  <div style="margin-top:20px;padding:0 4px;">
    ...
    ${unsubLine}
    <p style="margin:0 0 8px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.55;color:#8a877f;">${esc(copy.questions)} ${esc(copy.email)} <a href="mailto:${esc(CONTACT_EMAIL)}" style="color:#473fb0;text-decoration:underline;">${esc(CONTACT_EMAIL)}</a>.</p>
    <p style="margin:0;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.55;color:#8a877f;">${protectBrand(esc(SENDER_NAME))}<br/>${esc(POSTAL_ADDRESS)}</p>
  </div>`;
}
  ...
  <div style="max-width:600px;margin:0 auto;padding:32px 20px;">
    <table role="presentation" style="border-collapse:collapse;"><tr>
      ...
      <td style="vertical-align:middle;">
        <div style="font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:20px;font-weight:800;letter-spacing:-0.01em;color:#16160f;line-height:1.2;">${title}</div>
        <div style="margin-top:2px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#8a877f;">${eyebrow}</div>
      </td>
    </tr></table>

    <div style="margin-top:22px;background:#ffffff;border:2px solid #473fb0;border-radius:20px;padding:30px;box-shadow:0 20px 48px -28px rgba(22,22,15,0.30);">
      ${opts.bodyHtml}
    </div>
